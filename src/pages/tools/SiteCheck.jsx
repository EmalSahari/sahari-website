...
function runHeadChecks(html, lang) {
  ...
  const labels = lang === 'da' ? {
    title: 'Title-tag',
    titleMissing: 'Tilføj et <title> i <head>, 50-60 tegn, med dit hovedsøgeord. Google bruger det som overskriften i søgeresultatet.',
    titleShort: (n) => `For kort (${n} tegn). Udvid til 50-60 og inkluder dit hovedsøgeord.`,
    titleLong: (n) => `For lang (${n} tegn). Forkort til under 60, ellers trunkerer Google den.`,
    titleOk: (n) => `${n} tegn, indenfor det optimale interval.`,
    desc: 'Meta description',
    descMissing: 'Tilføj en meta description på 120-160 tegn i <head>. Det er den tekst Google viser under din titel i søgeresultatet, så skriv den som en salgspitch.',
    descShort: (n) => `For kort (${n} tegn). Udvid til 120-160 så du har plads til at overbevise søgeren om at klikke.`,
    descLong: (n) => `For lang (${n} tegn). Forkort til under 160, ellers trunkerer Google den.`,
    descOk: (n) => `${n} tegn, indenfor det optimale interval.`,
    h1: 'H1 overskrift',
    h1Missing: 'Tilføj ét <h1> øverst på siden der beskriver hvad siden handler om. Det skal være sidens hovedoverskrift.',
    h1Multiple: (n) => `${n} H1-tags fundet. Ændr alle undtagen ét til H2 eller H3, kun ét H1 per side.`,
    h1Ok: 'Præcis ét H1-tag.',
    canonical: 'Canonical-tag',
    canonicalMissing: 'Tilføj <link rel="canonical" href="fuld URL"> i <head>. Det fortæller Google hvilken version af siden der er den primære og forhindrer duplicate content-problemer.',
    canonicalOk: 'Canonical-tag til stede.',
    lang: 'Lang-attribut på <html>',
    langMissing: 'Tilføj lang="da" (eller relevant sprogkode) på <html>-tagget. Fortæller browsere og søgemaskiner sproget.',
    langOk: (l) => `Sat til "${l}".`,
    viewport: 'Mobile viewport',
    viewportMissing: 'Tilføj <meta name="viewport" content="width=device-width, initial-scale=1"> i <head>. Uden den ser siden meget lille ud på mobil, og Google straffer det i ranking.',
    viewportOk: 'Viewport meta-tag til stede.',
    og: 'Open Graph tags',
    ogMissing: 'Tilføj og:title, og:description og og:image meta-tags i <head>. De styrer hvordan dit link ser ud når det deles på LinkedIn, iMessage, Slack og WhatsApp.',
    ogPartial: (m) => `Mangler: ${m.join(', ')}. Tilføj dem så delte links viser ordentligt preview.`,
    ogOk: 'og:title, og:description og og:image er til stede.',
    twitter: 'Twitter/X kort',
    twitterMissing: 'Tilføj <meta name="twitter:card" content="summary_large_image"> i <head>. Sikrer at linket viser et stort billede når det deles på X.',
    twitterOk: 'Twitter card meta-tag til stede.',
    structuredData: 'Structured data (JSON-LD)',
    structuredMissing: 'Tilføj en JSON-LD schema (fx Organization eller LocalBusiness) i en <script type="application/ld+json"> i <head>. Det gør siden berettiget til rich results i Google.',
    structuredOk: (n) => `${n} JSON-LD blok${n === 1 ? '' : 'ke'} fundet.`,
    altText: 'Alt-tekst på billeder',
    altNone: 'Ingen billeder på siden.',
    altLow: (p, t) => `${p} ud af ${t} billeder mangler alt-tekst. Tilføj en kort beskrivelse til hvert billede, hjælper skærmlæsere og giver Google ekstra kontekst.`,
    altOk: (t) => `Alle ${t} billeder har alt-tekst.`,
    words: 'Indholdsmængde',
    wordsLow: (n) => `Kun ~${n} ord på siden. Sider under 200 ord ranker sjældent, udvid med tekst der forklarer hvad du tilbyder, hvem det er for, og hvad kunden får ud af det.`,
    wordsOk: (n) => `~${n} ord, godt grundlag for SEO.`,
  } : {
    title: 'Title tag',
    titleMissing: 'Add a <title> tag in <head>, 50-60 characters, including your main keyword. Google uses it as the heading in the search result.',
    titleShort: (n) => `Too short (${n} chars). Expand to 50-60 and include your main keyword.`,
    titleLong: (n) => `Too long (${n} chars). Trim to under 60, otherwise Google truncates it.`,
    titleOk: (n) => `${n} chars, within the sweet spot.`,
    desc: 'Meta description',
    descMissing: 'Add a meta description of 120-160 characters in <head>. It is what Google shows under your title in search results, so write it as a sales pitch.',
    descShort: (n) => `Too short (${n} chars). Expand to 120-160 so you have space to convince the searcher to click.`,
    descLong: (n) => `Too long (${n} chars). Trim to under 160, otherwise Google truncates it.`,
    descOk: (n) => `${n} chars, within the sweet spot.`,
    h1: 'H1 heading',
    h1Missing: 'Add a single <h1> at the top of the page describing what the page is about. It should be the main heading.',
    h1Multiple: (n) => `${n} H1 tags found. Change all but one to H2 or H3, only one H1 per page.`,
    h1Ok: 'Exactly one H1 tag.',
    canonical: 'Canonical tag',
    canonicalMissing: 'Add <link rel="canonical" href="full URL"> in <head>. It tells Google which version of the page is canonical and prevents duplicate content issues.',
    canonicalOk: 'Canonical tag present.',
    lang: 'Lang attribute on <html>',
    langMissing: 'Add lang="en" (or your language code) on the <html> tag. Tells browsers and search engines the page language.',
    langOk: (l) => `Set to "${l}".`,
    viewport: 'Mobile viewport',
    viewportMissing: 'Add <meta name="viewport" content="width=device-width, initial-scale=1"> in <head>. Without it the page looks tiny on phones, which Google penalises in mobile rankings.',
    viewportOk: 'Viewport meta tag present.',
    og: 'Open Graph tags',
    ogMissing: 'Add og:title, og:description and og:image meta tags in <head>. They control how the link looks when shared on LinkedIn, iMessage, Slack and WhatsApp.',
    ogPartial: (m) => `Missing: ${m.join(', ')}. Add them so shared links show a proper preview.`,
    ogOk: 'og:title, og:description and og:image all present.',
    twitter: 'Twitter/X card',
    twitterMissing: 'Add <meta name="twitter:card" content="summary_large_image"> in <head>. Makes the link show a big image when shared on X.',
    twitterOk: 'Twitter card meta tag present.',
    structuredData: 'Structured data (JSON-LD)',
    structuredMissing: 'Add a JSON-LD schema (e.g. Organization or LocalBusiness) inside a <script type="application/ld+json"> in <head>. It makes the page eligible for rich results in Google.',
    structuredOk: (n) => `${n} JSON-LD block${n === 1 ? '' : 's'} found.`,
    altText: 'Image alt text',
    altNone: 'No images on the page.',
    altLow: (p, t) => `${p} of ${t} images missing alt text. Add a short description to each image, which helps screen readers and gives Google extra context.`,
    altOk: (t) => `All ${t} images have alt text.`,
    words: 'Content depth',
    wordsLow: (n) => `Only ~${n} words on the page. Pages under 200 words rarely rank, expand with content explaining what you offer, who it is for, and what the customer gets out of it.`,
...
async function runDiscoveryChecks(targetUrl, lang) {
  ...
  const labels = lang === 'da' ? {
    robots: 'robots.txt',
    robotsMissing: 'Opret en /robots.txt fil i roden af dit domæne med mindst `User-agent: *\nAllow: /\nSitemap: https://ditdomæne.dk/sitemap.xml`. Den vejleder crawlere om hvad de må indeksere.',
    robotsOk: 'Findes på /robots.txt.',
    sitemap: 'Sitemap.xml',
    sitemapMissing: 'Generer en /sitemap.xml der lister alle dine vigtige sider, og indsend den i Google Search Console. Det hjælper Google med at opdage og indeksere alt på siden.',
    sitemapOk: 'Findes på /sitemap.xml.',
  } : {
    robots: 'robots.txt',
    robotsMissing: 'Create a /robots.txt file at the root of your domain with at minimum `User-agent: *\nAllow: /\nSitemap: https://yourdomain.com/sitemap.xml`. It guides crawlers on what they can index.',
    robotsOk: 'Present at /robots.txt.',
    sitemap: 'Sitemap.xml',
    sitemapMissing: 'Generate a /sitemap.xml listing all your important pages and submit it in Google Search Console. Helps Google discover and index everything on the site.',
